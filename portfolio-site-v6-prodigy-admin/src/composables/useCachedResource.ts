...
/** Loads JSON from `url`, caching it in localStorage under `key` for `ttlMs`. Mirrors the UI's composable of the same name - keep in sync. */
export function useCachedResource<T>(key: string, url: string, ttlMs = ONE_WEEK_MS) {
  ...
  async function load() {
    // No localStorage during Astro's server prerender - real load happens on client hydration.
